...
class NikkeCNFixService {
    private let shellService = ShellService.shared
    private let bundleIdentifier = "com.tencent.nikke"
    
    func fixDisplay() async -> (success: Bool, output: String, error: String) {
        let executable = "~/Library/Containers/io.playcover.PlayCover/Applications/com.tencent.nikke.app/Frameworks/UnityFramework.framework/UnityFramework"
        let expandedPath = await shellService.expandPath(executable)
        
        let exists = await shellService.checkFileExists(at: expandedPath)
        guard exists else {
            return (false, "", "未找到UnityFramework文件，请确认胜利女神：新的希望已通过Mac俱乐部客户端安装")
        }
...
"""
    ...
    func fixRedeemCode() async -> (success: Bool, output: String, error: String) {
        let executable = "~/Library/Containers/io.playcover.PlayCover/Applications/com.tencent.nikke.app/Frameworks/MSDKPIXWebView.framework/MSDKPIXWebView"
        let expandedPath = await shellService.expandPath(executable)
        
        let exists = await shellService.checkFileExists(at: expandedPath)
        guard exists else {
            return (false, "", "未找到MSDKPIXWebView框架文件，请确认胜利女神：新的希望已通过Mac俱乐部客户端安装")
        }
        ...
        let funcAddr = parseOtoolOutput(otoolResult.output,
                                        className: "MSDKPIXWKWebViewController",
                                        methodName: "supportedInterfaceOrientations")
...
"""
    ...
    func enableLimitMotionUpdateFrequency() async -> (success: Bool, output: String, error: String) {
        ...
        guard let app = apps.first(where: { $0.info.bundleIdentifier == bundleIdentifier }) else {
            return (false, "", "未找到胜利女神：新的希望，请确认游戏已通过Mac俱乐部客户端安装\n\nBundle ID: \(bundleIdentifier)")
        }
...
已为胜利女神：新的希望启用 limitMotionUpdateFrequency 选项。
