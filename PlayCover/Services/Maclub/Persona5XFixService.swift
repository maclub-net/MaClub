...
class Persona5XFixService {
    private let shellService = ShellService.shared
    private let bundleIdentifier = "com.pwrd.persona5x.pw"
    
    func fixStartupCrash() async -> (success: Bool, output: String, error: String) {
        let executable = "~/Library/Containers/io.playcover.PlayCover/Applications/com.pwrd.persona5x.pw.app/Frameworks/UnityFramework.framework/UnityFramework"
        let expandedPath = await shellService.expandPath(executable)
        
        let exists = await shellService.checkFileExists(at: expandedPath)
        guard exists else {
            return (false, "", "未找到UnityFramework文件，请确认女神异闻录：夜幕魅影已通过Mac俱乐部客户端安装")
        }
...
"""
    ...
    func enableLimitMotionUpdateFrequency() async -> (success: Bool, output: String, error: String) {
        ...
        guard let app = apps.first(where: { $0.info.bundleIdentifier == bundleIdentifier }) else {
            return (false, "", "未找到女神异闻录：夜幕魅影，请确认游戏已通过Mac俱乐部客户端安装\n\nBundle ID: \(bundleIdentifier)")
        }
...
已为女神异闻录：夜幕魅影启用 limitMotionUpdateFrequency 选项。
